...
<div class=" flex w-screen h-screen relative justify-center items-center">
    ...
    <form class=" w-1/2 p-4 shadow-2xl rounded-lg border-2 border-solid" action="{{ route('yeseditstock') }}" method="post" enctype="multipart/form-data">
        @csrf
        <div class=" flex justify-center">
            <p class=" text-4xl">แก้ไขวัตถุดิบ</p>
        </div>
        <div class=" w-full my-4">
            <p class=" text-xl ml-2">ชื่อวัตถุดิบ</p>
            <input class=" flex  text-lg border-solid rounded-lg border-2 border-gray-700 w-full my-2 py-2 px-3" type="text" name="name" value="{{$stock->name}}">
        </div>
        <div class=" w-full my-4">
            <p class=" text-xl ml-2">จำนวน</p>
            <input class=" flex text-lg border-solid rounded-lg border-2 border-gray-700 w-full my-2 py-2 px-3" type="number" step="0.01" name="amount" value="{{$stock->amount}}">
            ...
            <select class=" flex text-lg border-solid rounded-lg border-2 border-gray-700 w-full my-2 py-2 px-3" name="s_unit_id">
                <option value="{{$stock->s_unit_id}}">{{$stock->uname}}</option>
                @foreach ($stock_units as $unit)
                    @if ($unit->name != $stock->uname)
            ...
            <select class=" flex text-lg border-solid rounded-lg border-2 border-gray-700 w-full my-2 py-2 px-3" name="s_type_id">
                <option value="{{$stock->s_type_id}}">{{$stock->tname}}</option>
                @foreach ($stock_types as $type)
                    @if ($type->name != $stock->tname)
